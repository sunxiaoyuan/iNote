...
@implementation SZYModel

#pragma mark - override method
+ (void)initialize
{
    if (self != [SZYModel self]) {
        [self createTable];
    }
}
...
{
    __block BOOL res = NO;
    SZYDBHelper *szyDB = [SZYDBHelper shareInstance];
    [szyDB.dbQueue inDatabase:^(FMDatabase *db) {
...
+ (BOOL)createTable
{
    FMDatabase *db = [FMDatabase databaseWithPath:[SZYDBHelper dbPath]];
    if (![db open]) {
        NSLog(@"数据库打开失败!");
        return NO;
    }
...
{
    //判断是否是JKBaseModel的子类
    for (SZYModel *model in array) {
        if (![model isKindOfClass:[SZYModel class]]) {
            return NO;
        }
    }
    ...
    SZYDBHelper *szyDB = [SZYDBHelper shareInstance];
    // 如果要支持事务
    [szyDB.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        for (SZYModel *model in array) {
...
+ (NSArray *)findAll
{
    ...
    [szyDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            SZYModel *model = [[self.class alloc] init];
            for (int i=0; i< model.columeNames.count; i++) {
