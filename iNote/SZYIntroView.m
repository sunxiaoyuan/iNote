...
@interface SZYIntroView() <UIScrollViewDelegate>
...
-(instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        [self.doneButton setHidden:YES]; //首先隐藏按钮，当最后一页时再显示出来
        
        //这里需要注意，添加子视图的时候，越加在后面的控件越在上层
    }
    return self;
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    //偏移量除屏幕宽，四舍五入之后，可以获得当前滚动到了第几页
    CGFloat pageFraction = self.scrollView.contentOffset.x / pageWidth;
    self.pageControl.currentPage = roundf(pageFraction);
    if (self.pageControl.currentPage == 3) {
        [self.doneButton setHidden:NO]; //最后一页显示按钮
    }else{
        [self.doneButton setHidden:YES]; //返回前面页面时，又要隐藏按钮
    }
    
}

#pragma mark - 重写getter方法，构建界面
